...
export const crearProveedor = (veterinariaNombre: string, nombre: string, telefono: string) => {
...
export const listarProveedores = (veterinariaNombre: string) => {
  ...
  if (datos.proveedores.length === 0) {
    ...
    datos.proveedores.forEach((prov: any) => console.log(`ID: ${prov.id} - Nombre: ${prov.nombre} - Teléfono: ${prov.telefono}`));
  }
};

export const modificarProveedor = (veterinariaNombre: string, proveedorId: string, nuevosDatos: Partial<{ nombre: string; telefono: string }>) => {
  const datos = cargarDatosVeterinaria(veterinariaNombre);
  const proveedor = datos.proveedores.find((prov: any) => prov.id === proveedorId);
...
export const eliminarProveedor = (veterinariaNombre: string, proveedorId: string) => {
  const datos = cargarDatosVeterinaria(veterinariaNombre);
  const indice = datos.proveedores.findIndex((prov: any) => prov.id === proveedorId);
